//Contantes globais
const nomeUser = document.getElementById('txtUser');
...
function paginacao(qRepos){
    resto = qRepos % 30; //resto da divisão se tornará uma página caso não seja igual a 0.
    if(resto != 0){
      resto = 1;  
    }
...
// Função que chama os repositorios e os configura, utilizando o do metódo getAPIDados
...
function main(){
    ...
    if(dados != false){
      ...
      if(usuario.name == null || usuario.name == ""){ // Verifica se o nome do usuário é vazio ou nulo, se for mostra o nome de login, senão mostra o nome de usuário.
